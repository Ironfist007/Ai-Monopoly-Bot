...
class GameUI:
    ...
    def __init__(self):
        ...
        self.window_width = self.board_width + self.panel_width + 30
    ...
    def _draw_property_info(self, rect, pos_idx, info):
        """Draw property value and ownership indicator"""
        # Find property by position index - use direct matching
        ...
        # If not found by name, skip (corner or special spaces)
        if prop is None:
            return
        
        # Draw price at bottom of space
        if info.get('value', 0) > 0:
            price_text = self.font_tiny.render(f"${info['value']}", True, (0, 120, 0))
    ...
    def _draw_player_stats(self, panel_x, y_offset, player, player_color, player_idx):
        ...
        # Player header with color indicator - show exact player ID from code
    ...
    def _draw_game_log(self, panel_x, y_offset):
        ...
        if len(expanded_lines) > max_visible_lines:
            ...
            handle_y_ratio = self.log_scroll_offset / max(1, max_scroll)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    self.log_scroll_offset = min(len(self.game_log) - 10, self.log_scroll_offset + 1)
                ...
                if event.button == 1:  # Left click
                    ...
                    self.log_scroll_offset = min(len(self.game_log) - 10, self.log_scroll_offset + 3)
